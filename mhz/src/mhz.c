...
loop_f loops[] = {
    mhz_1, mhz_2, mhz_3, mhz_4, mhz_5, mhz_6, mhz_7, mhz_8, mhz_9,
};
 ...
 */
int filter_data(double values[], int size) {
    int     i;
    int     tests;
    double  median;
 ...
 * data subset.
 */
int classes(double values[], int size) {
    int     i;
    double  median;
 ...
 * of data points.
 */
void cross_values(double values[], int size, double **cvalues, int *csize) {
    int i, j;
 ...
 */
double gcd(double values[], int size) {
...
int compute_mhz(result_t *r) {
    ...
    if (ABS(mhz[0] - mhz[1]) / (double)mhz[0] <= 0.01 || ABS(mhz[0] - mhz[1]) <= 1) return mhz[0];
...
void print_data(double mhz, result_t *data) {
    int   i, j;
    char *CPU_name = "CPU";
    char *uname    = "uname";
    char *email    = "email";
